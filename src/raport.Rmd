...
```{r echo=FALSE, fig.height=3.4, fig.align='center'}
    genieResults <- lapply(fileNames, Compare_method, methodName="genie");
    df <- Reduce(rbind, genieResults);
    plotComparedMethod(df, "Genie");
```

# Indeksy FM i AR dla pakietu protoclust
Pakiet `protoclust`, dostępny na [CRAN](https://cran.r-project.org/web/packages/protoclust/index.html), udostępnia funkcję `protoclust`, która dokonuje analizy skupień na zadanym zbiorze danych. Zgodnie ze specyfikacją, algorytm osadza przetwarzane punkty w przestrzeni k największych wartości własnych znormlizowanej macierzy podobieństwa.

> Performs minimax linkage hierarchical clustering given a set of dissimilarities.
...
```{r fig.width=6, fig.height=3.4, echo=FALSE, fig.align='center'}
    protoclustResults <- lapply(fileNames, Compare_method, methodName="protoclust");
    df <- Reduce(rbind, protoclustResults);
    plotComparedMethod(df, "protoclust");
```

Jak widać, `protoclust:protoclust` osiąga pełną zgodność z wzorcowymi skupieniami dla niektórych zbiorów (po głębszej analize - zadowalające wyniki dla 26/44 zbiorów). Minusem opisywanego algorytmu jest jego szybkość działania - średnia szybkość wykonania analizy to 7 sekund.
...
```{r fig.width=6, fig.height=3.4, echo=FALSE, fig.align='center'}
    customResults <- lapply(fileNames, Compare_method, methodName="custom_5");
    df <- Reduce(rbind, customResults);
    plotComparedMethod(df, "M = 5");
```

```{r fig.width=6, fig.height=3.4, echo=FALSE, fig.align='center'}
    customResults <- lapply(fileNames, Compare_method, methodName="custom_20");
    df <- Reduce(rbind, customResults);
    plotComparedMethod(df, "M = 20");
```

```{r fig.width=6, fig.height=3.4, echo=FALSE, fig.align='center'}
    customResults <- lapply(fileNames, Compare_method, methodName="custom_100");
    df <- Reduce(rbind, customResults);
    plotComparedMethod(df, "M = 100");
```

```{r fig.width=6, fig.height=3.4, echo=FALSE, fig.align='center'}
    customResults <- lapply(fileNames, Compare_method, methodName="custom_200");
    df <- Reduce(rbind, customResults);
    plotComparedMethod(df, "M = 200");
```

```{r fig.width=6, fig.height=3.4, echo=FALSE, fig.align='center'}
    customResults <- lapply(fileNames, Compare_method, methodName="custom_500");
    df <- Reduce(rbind, customResults);
    plotComparedMethod(df, "M = 500");
```
# Wnioski
